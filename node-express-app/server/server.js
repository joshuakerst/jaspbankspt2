...
app.get("/banks", (req, res) => {
	//creates a string to hold all the HTML stuff
	let html = "<h1>Banks for Jasper</h1> <ul>";

	//loops through array and creates a li inside the ul, also adds button that alerts code, appends to html
	banks.forEach((bank) => {
		html += `<li>${bank.bank_name} <button onclick="alert('The Bank Code is ${bank.code}')">  Code  </button></li>`;
	});
